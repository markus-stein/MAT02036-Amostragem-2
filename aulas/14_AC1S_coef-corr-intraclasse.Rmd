...
$$\rho = \frac{Cov(Y_1', Y_2')}{\sqrt{Var(Y_1')Var(Y_2')}}$$
  + $Y_1'$ significa possíveis valores da da primeira posição do par 
  + $Y_2'$ significa a segunda posição do par.
...
* Sob **AASs** dentro dos conglomerados, de acordo com @Cochran1977, página 242, tem-se:
...
$$S_i^2 = \frac {1} {\overline N - 1} \sum_{j \in C_i} \left( y_{ij} - \overline {Y_i} \right)^2$$ 

---

## Coeficiente de Correlação Intraclasse

* Lembrando, variância **total** é dada por: 

$$S_y^2 = \frac {1} {N - 1} \sum_{i \in C} \sum_{j \in C_i} \left( y_{ij} - \overline Y \right)^2$$

* Vimos que a variância **total** também pode ser expressa em função das variâncias *entre* conglomerados, $S_e^2$, e *dentro* dos conglomerados, $S_d^2$, através da expressão:

$$S_y^2 = \frac {(\overline N - 1) M S_{dc}^2 + \overline N (M-1) \overline S_{ec}^2} {M \overline N - 1}$$
onde, $\overline S_{ec}^2 = \frac {S_e^2} {\overline N}$.
...
**2.** Pode ser demonstrado (`r emo::ji("question")`) que $\left [ - \left( \frac {M-1} {N-M} \right)\right] \le \rho \le 1$, ou ainda, de forma aproximada, que $\frac {-1} {\overline N - 1} \le \rho \le1$.    
...
**4.** Num caso extremo, $\rho = 1$ e portanto $EPA(AC1S^R;\,AAS) = \overline N$ e $Var_{AC1S} (\overline y_{AC1S}^R) = EPA(AC1S^R;\,AAS)\, Var_{AAS} (\overline y) \doteq \overline N \frac {S_y^2} {m \overline N} = \frac{S_y^2}{m}$    
