...
a) Usando o meu no. cartão a amostra **AC1Ss** de 15 estados (variável `state`) é
...
b. A partir da amostra sorteada, uma estimativa pontualmente da média... de fazendas... `largef92`.  
...
Ni_amostra <- aggregate( largef92 ~ state, data=agpop_amostra, length) # ou tapply(), tamanho dos cong.
Ti_amostra <- aggregate( largef92 ~ state, data=agpop_amostra, sum)    # totais cong.
ybarraAC1S <- (M/m) * sum(Ti_amostra$largef92) / N
#ybarrac
#s2ec
#varybarraC
```
Assim, estimamos que... é aproximadamente `r ybarraAC1S`.
