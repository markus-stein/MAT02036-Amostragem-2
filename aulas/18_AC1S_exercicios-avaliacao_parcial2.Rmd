...
O coeficiente de correlação intraclasse (CCI) para a divisão $A$ é aproximadamente $`r CA`$. O valor mínimo para o CCI é $-\frac{1}{\overline N - 1} = `r round(-1/(Ni - 1),3)`$, o que indica uma grande ineficiência da $AC1S$ usando a conglomeração $A$ em relação a **AASc**, pois $\rho_{int}$ está próximo de 1.
...
* $T_i$ é o número de pneus (dentre os 4 pneus em uso) que estavam fora de condições de segurança. 
...
$$\widehat P_{AC1S}^{HT} = \frac {\widehat T_{AC1S/HT}}{N} = \frac {M} {N} \frac{1}{m} \sum_{i \in a} T_i = \frac{ \frac{`r M`}{`r m`} `r sum(Ti)`}{`r N`} = `r (M/m) * sum(Ti) / N`$$
ou
$$\widehat P_{AC1S}^R = \frac{\widehat T_{AC1S}^R}{N} = \frac{1}{n} \sum_{i \in a} T_i = \frac{ \frac{`r N`}{`r n`} `r sum(Ti)`}{`r N`} = `r sum(Ti) / n`.$$

**Estimamos** que a **proporção de pneus fora das condições** de segurança é aproximadamente `r (M/m) * sum(Ti) / N` em cada carro da empresa investigada.
...
Com os dados do problema $\overline y_C = \frac{\sum_{1 \in a} T_i}{m} = \frac{`r sum(Ti)`}{`r m`} = `r sum(Ti)/m`$ e 
$$s^2_{ec} = $$
...
b) Usando os resultados como um estudo piloto, o número de táxis necessário para obter uma estimativa da proporção de pneus fora das condições, com um erro absoluto de $2,5\%$ e $95\%$ de confiança, considerando **AC1s** com reposição, ...
