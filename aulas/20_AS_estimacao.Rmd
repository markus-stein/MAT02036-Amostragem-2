...
* Para estimar a média populacional $\overline Y = \frac Y N = \frac {\sum_{r=1}^{K} t_r} {\sum_{r=1}^{K} n_r}$ um estimador não viciado (`r emo::ji("question")`) é dado por (quando $N$ é conhecido)
$$\overline y_{AS} = \frac {\widehat Y_{AS}} N = \frac {K t_r}{N}.$$
...
$$\overline y_{AS}=\frac {\widehat Y_{AS}}{\widehat N_{AS}}=\frac {Kt_r}{Kn_r}=\frac {t_r}{n_r}=\overline y_r=\overline y$$
...
onde: $\overline t = \displaystyle \frac 1 K \sum_{r=1}^{K} t_r = \frac T K$
...
V_YhatAS_ord <- K * (var(tr) * (K-1))    # variancia do estimador do total
V_YhatAS_ord
```
...
1. Como $r$ pode tomar apenas um valor, $Var_{AS}(\widehat Y_{AS})$ não pode ser diretamente estimada a partir da amostra.    
...
$$Var_{AS}(\overline y_{AS})  = \left( \frac {N-1} {N} \right) \left[ 1 + (n-1) \rho \right] \frac {S_y^2} {n} \doteq \left[ 1 + (n-1)\rho \right] \frac {S_y^2} {n},$$ 
onde
...
$$\widehat V_{1AS} (\widehat Y_{AS}) =  N^2\widehat V_{1AS} (\overline y_{AS})$$
...
$$\widehat V_{2AS} (\widehat Y_{AS}) =  N^2\widehat V_{2AS} (\overline y_{AS})$$
...
$\widehat V_{1AS} (\widehat T_{AS}) = N^2\widehat V_{1AS} (\overline y_{AS})$ | se $N$ é conhecido e não houver ordenação das unidades na população
$\widehat V_{2AS}(\widehat T_{AS}) = N^2\widehat V_{2AS}(\overline y_{AS})$ | se $N$ é conhecido e houver ordenação das unidades na população
$\widehat V_{1AS} (\overline y_{AS}) = \left(\frac {1}{n} - \frac 1 {N} \right) \frac {1}{n-1} \sum_{i \in s_r} (y_i - \overline y_{AS})^2$ | se  $N$ é conhecido e não houver ordenação das unidades na população}
$\widehat V_{2AS}(\overline y_{AS}) = \left( \frac {1}{n} - \frac 1 {N} \right) \frac {1}{2(n-1)} \sum_{i \in s_r} (y_i - y_{i+K})^2$ | se $N$ é conhecido e houver ordenação das unidades na população
