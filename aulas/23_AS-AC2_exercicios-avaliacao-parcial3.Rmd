...
```
a) A variável de interesse, $y$, (observada em cada uma das unidades elementares) é a distância (em milhas) percorrida pelas aéronaves das companhias aéreas de um país. 
...
* Para o total temos 
  + $\widehat T^{HT}_{AC2S} = \frac {M} {m} \sum_{i \in a} \widehat T_i$, $\widehat T_i = \frac {N_i} {n_i} \sum_{j \in s_i} y_{ij}$, ou 
  + $\widehat T^R_{AC2S} = N \frac{ \sum_{i \in a} \widehat T_i}{ \sum_{i \in a} N_i}$;
